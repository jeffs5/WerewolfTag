...
class Controller():
    def __init__(self, model, network):
        self.m = model
        self.n = network
        self.controls = {pygame.K_LEFT : (-1,0), pygame.K_RIGHT : (1,0), pygame.K_UP : (0,-1), pygame.K_DOWN : (0,1), pygame.K_c : (1,1)} # player controls
                    ...
                    for pressed in self.controls:
                        if key[pressed]:
                            if pressed == pygame.K_c: 
                                ...
                                n.do_send({'place': True, 'x': random.randint(0, self.m.board_x), 'y': random.randint(0, self.m.board_y), 'name': placeable})
                            else:
                                instruction = self.controls.get(pressed)
                                moving_player = self.m.players[str(self.m.player_number)]
